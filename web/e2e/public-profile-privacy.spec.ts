...
test("canonical public profile renders public data without private library details", async ({
  page,
  api,
}) => {
  await signIn(page);
  ...
  await expect
    .poll(() => api.requests.some((request) => request.path === "/users/public-player"))
    .toBe(true);
...
test("self and friend profile variants use the same canonical public route", async ({
  page,
  api,
}) => {
  api.state.publicProfiles["me"] = {
    ...
    favorites: { status: "ready", data: [] },
    wishlist: { status: "ready", data: [] },
  };
  ...
  api.state.friendProfiles.friend = {
    user: { id: "friend-1", public_id: "friend", display_name: "Sam" },
    library: { status: "ready", data: [] },
  };
  await signIn(page);
  ...
  await expect
    .poll(() => api.requests.some((request) => request.path === "/users/friend/friend-profile"))
    .toBe(true);
