...
export async function installGuestHomeRoutes(page: Page): Promise<ApiRoutes> {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (request.method() === "GET" && path === "/friends") { await route.fulfill({ json: state.friends }); return; }
    if (request.method() === "GET" && path === "/friends/requests/incoming") { await route.fulfill({ json: state.incomingFriendRequests }); return; }
    if (request.method() === "GET" && path === "/steam/social") {
      await route.fulfill({ json: { friends: [], friends_total: 0, friends_has_more: false } }); return;
    }
    if (request.method() === "GET" && path === "/users/search") { await route.fulfill({ json: state.users }); return; }
    if (request.method() === "POST" && path === "/friends/requests") {
      await route.fulfill({ json: { id: "request-created", ...requestRecord.jsonBody } }); return;
    }
    if (request.method() === "POST" && path === "/social/friend-requests") {
      await route.fulfill({ json: { id: "request-created", ...requestRecord.jsonBody } }); return;
    }
    if (request.method() === "POST" && /^\/friends\/requests\/[^/]+\/accept$/.test(path)) {
      await route.fulfill({ json: state.friends[0] ?? { user: state.users[0] } }); return;
    }
    if (request.method() === "GET" && path === "/conversations") { await route.fulfill({ json: state.conversations }); return; }
    if (request.method() === "POST" && path === "/conversations") {
      const friendId = (requestRecord.jsonBody as { recipient_id: string }).recipient_id;
      const participant = state.friends.find((friend) => friend.user.id === friendId)?.user ?? state.users[0];
      const conversation = { id: "conversation-created", participant, updated_at: "2026-08-21T00:00:00Z" };
      state.conversations.push(conversation); await route.fulfill({ json: conversation }); return;
    }
    const conversationMessages = path.match(/^\/conversations\/([^/]+)\/messages$/);
    if (conversationMessages && request.method() === "GET") { await route.fulfill({ json: state.messages[conversationMessages[1]] ?? [] }); return; }
    if (conversationMessages && request.method() === "POST") {
      const message = { id: "message-created", sender_id: "user-1", body: (requestRecord.jsonBody as { body: string }).body, created_at: "2026-08-21T00:00:00Z" };
      state.messages[conversationMessages[1]] = [...(state.messages[conversationMessages[1]] ?? []), message];
    ...
    if (request.method() === "POST" && /^\/game-invites\/[^/]+\/response$/.test(path)) {
      ...
      const response = { ...invite, status: (requestRecord.jsonBody as { status: "accepted" | "declined" }).status };
      await route.fulfill({ json: response }); return;
    }
    if (request.method() === "GET" && path === "/notifications") { await route.fulfill({ json: state.notifications }); return; }
    if (request.method() === "POST" && /^\/notifications\/[^/]+\/read$/.test(path)) {
      const id = path.split("/")[2];
      const notification = state.notifications.find((item) => item.id === id);
      if (notification) notification.read_at = "2026-08-21T00:00:00Z";
      await route.fulfill({ json: notification ?? {} }); return;
    }
    if (request.method() === "POST" && path === "/notifications/read-all") {
      state.notifications.forEach((notification) => { notification.read_at = "2026-08-21T00:00:00Z"; });
    ...
    if (publicProfile && request.method() === "GET") {
      ...
      if (!profile) { await route.fulfill({ status: 404, json: { detail: "Not found" } }); return; }
    ...
    if (sharedLibrary && request.method() === "GET") { await route.fulfill({ json: state.sharedLibraries[sharedLibrary[1]] ?? { status: "empty", data: [] } }); return; }
