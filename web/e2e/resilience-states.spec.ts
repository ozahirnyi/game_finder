...
test("friends does not render an empty count while its request is pending", async ({
  page,
  api,
}) => {
...
test("failed profile and authenticated 401 preserve a usable application shell", async ({
  page,
  api,
}) => {
  ...
  await expect
    .poll(() => api.requests.some((request) => request.path === "/users/missing-player"))
    .toBe(true);
