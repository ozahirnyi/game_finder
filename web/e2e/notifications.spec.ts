...
test("a valid notification deep link marks only its target read", async ({ page, api }) => {
  api.state.notifications = [
    {
      id: "n-request",
      type: "friend_request",
      payload: { from: "Sam", request_id: "request-1" },
      created_at: "2026-08-21T00:00:00Z",
    },
    {
      id: "n-message",
      type: "message",
      payload: { from: "Pat", conversation_id: "missing" },
      created_at: "2026-08-21T00:00:00Z",
    },
  ];
  api.state.incomingFriendRequests = [
    {
      id: "request-1",
      sender: { id: "friend-1", public_id: "sam", display_name: "Sam" },
  ...
  await expect
    .poll(
      () =>
        api.requests.filter((request) => request.path === "/notifications/n-request/read").length,
    )
    .toBe(1);
  await expect
    .poll(
      () =>
        api.requests.filter((request) => request.path === "/notifications/n-message/read").length,
    )
    .toBe(0);
});

test("malformed notification targets stay unread and show a neutral unavailable message", async ({
  page,
  api,
}) => {
  api.state.notifications = [
    { id: "n-bad", type: "game_invite", payload: {}, created_at: "2026-08-21T00:00:00Z" },
  ];
  await signIn(page);
  await page.goto("/account");
  await waitForHydration(page);
...
const notificationDestinations = [
  ...
  [
    "game_invite",
    "Game invite",
    { from: "Sam", game_name: "Celeste", invite_id: "invite-1" },
    "**/friends?invite=invite-1&notification=n-target",
  ],
  [
    "game_invite_response",
    "Game invite response",
    { by: "Sam", invite_id: "invite-1", status: "accepted" },
    "**/friends?invite=invite-1&notification=n-target",
  ],
  [
    "price_alert",
...
for (const [type, title, payload, url] of notificationDestinations) {
  test(`${type} deep link uses its canonical destination`, async ({ page, api }) => {
    api.state.notifications = [
      { id: "n-target", type, payload, created_at: "2026-08-21T00:00:00Z" },
    ];
    ...
    if (type === "message")
      api.state.conversations = [
        {
          id: "conversation-1",
          participant: { id: "friend-1", public_id: "sam", display_name: "Sam" },
          updated_at: "2026-08-21T00:00:00Z",
        },
      ];
    if (type === "game_invite" || type === "game_invite_response")
...
test("deleted friend-request targets stay unread and report neutral unavailability", async ({
  page,
  api,
}) => {
  api.state.notifications = [
    {
      id: "n-deleted",
      type: "friend_request",
      payload: { request_id: "deleted", from: "Sam" },
      created_at: "2026-08-21T00:00:00Z",
    },
  ];
  await signIn(page);
...
test("unauthorized friend-request targets stay unread and report neutral unavailability", async ({
  page,
  api,
}) => {
  api.state.notifications = [
    {
      id: "n-unauthorized",
      type: "friend_request",
      payload: { request_id: "request-1", from: "Sam" },
      created_at: "2026-08-21T00:00:00Z",
    },
  ];
