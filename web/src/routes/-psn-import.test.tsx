...
async function upload(container: HTMLElement) {
  await screen.findByText("Choose an export file");
  fireEvent.change(container.querySelector('input[type="file"]')!, {
    target: { files: [new File(["x"], "export.xlsx")] },
  });
  await screen.findByText("Games to import (3)");
}
...
describe("PsnImportPage", () => {
  ...
  it("disables confirmation while import is in flight", async () => {
    ...
    confirmPsnImport.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
