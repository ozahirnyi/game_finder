...
export function Dashboard() {
  const trendingQuery = useQuery({ queryKey: lovableQueryKeys.trendingGames(3), queryFn: () => getTrendingGames(3) });
  const catalog = (trendingQuery.data?.results.map(toGameCard) ?? []).map((game) => ({ id: game.id ?? "data-unavailable", title: game.title ?? "Data unavailable", coverFrom: game.imageUrl ?? "Data unavailable", coverTo: game.imageUrl ?? "Data unavailable", genres: [] as string[] }));
  const featured = catalog[0] ?? { id: "data-unavailable", title: "Data unavailable", coverFrom: "Data unavailable", coverTo: "Data unavailable", genres: [] as string[] };
  const shared = catalog.slice(1);
  const online: Array<{ id: string; avatarFrom: string; avatarTo: string; name: string; online: boolean; activity: string; lft: boolean; compatibility: number }> = [];
  const wishlistDeals: typeof catalog = [];
  const recs = catalog.map((game) => ({ gameId: game.id, game, reason: "Data unavailable" }));
  const activity: Array<{ id: string; who: string; verb: string; target: string; tag: string; time: string }> = [];
        ...
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-muted-foreground">
              Browse titles from the public game catalog.
            </p>
          </div>
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-background via-background/80 to-transparent p-6">
              <div className="mb-2 flex items-center gap-2">
                <Chip tone="primary">Trending</Chip>
                <Chip tone="outline">Data unavailable</Chip>
              </div>
        ...
        <div className="space-y-12 lg:col-span-8">
          ...
          <section className="animate-reveal" style={{ animationDelay: "100ms" }}>
            <SectionHeader
              ...
              hint="Titles returned by the public catalog."
              action={
                <Link
                  to="/search"
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <span className="font-mono text-sm">Data unavailable</span>
                    </div>
                  </div>
                </Link>
              ))}
        ...
        <div className="space-y-10 lg:col-span-4">
          <section className="animate-reveal" style={{ animationDelay: "200ms" }}>
            ...
            <div className="space-y-2">
              ...
              )) : <p className="rounded-xl border border-transparent p-3 text-sm text-muted-foreground">Connect Steam to view friends.</p>}
            </div>
          </section>
          ...
          <section className="relative overflow-hidden rounded-2xl border border-border bg-surface p-6">
            ...
            <div className="relative">
              ...
              <p className="mb-4 text-sm text-muted-foreground">
                Connect your account to view your library.
              </p>
              <Link
                to="/steam"
              ...
              >
                Connect Steam <ArrowRight className="size-3.5" />
              </Link>
            </div>
          </section>
          ...
          <section className="animate-reveal" style={{ animationDelay: "250ms" }}>
            ...
            <button disabled title="Activity posting is unavailable" className="mt-4 inline-flex items-center gap-2 text-xs font-bold text-muted-foreground hover:text-foreground">
              <Plus className="size-3" /> Data unavailable
            </button>
          </section>
        </div>
      </div>
