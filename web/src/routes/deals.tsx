...
function DealCard({ deal, large = false }: { deal: Deal; large?: boolean }) {
  ...
  return <div className="relative">
    ...
    <div className={`hover-lift group flex h-full ${large ? "flex-col" : "items-center"} gap-4 rounded-2xl border border-border bg-surface p-4 hover:border-primary/40`}>
      <GameCover from="#dc2626" to="#111827" title={deal.name} image={deal.background_image ?? undefined} compact={!large} className={large ? "aspect-[4/3] w-full rounded-xl" : "size-20 shrink-0 rounded-xl"} />
      ...
      <div className={`${large ? "relative z-20 flex items-end justify-between border-t border-border pt-3" : "text-right"}`}>{deal.current?.cut != null && <Chip tone="primary">-{deal.current.cut}%</Chip>}<div className={large ? "text-right" : ""}><p className="font-mono text-[10px] text-muted-foreground line-through">{deal.current?.regular ? `${deal.current.regular.currency} ${deal.current.regular.amount}` : "—"}</p><p className="font-mono text-lg font-black text-primary">{deal.current?.price ? `${deal.current.price.currency} ${deal.current.price.amount}` : "—"}</p></div></div>
    </div>
  </div>;
}
...
function DealsPage() {
  ...
  return <AppShell>
    ...
    <section className="mb-10"><SectionHeader title="Popular on Steam" hint={`${data.popular.length} discounts`} /><div className="mt-4 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">{data.popular.slice(0, 4).map((deal) => <DealCard key={`${deal.id ?? deal.steam_appid ?? deal.name}`} deal={deal} large />)}</div></section>
    {activeSection && <section><SectionHeader title="Browse by genre" hint="Choose a genre" /><div className="mt-4 flex flex-wrap gap-2">{sections.map((section) => <button key={section.genre} type="button" aria-pressed={activeGenre === section.genre} onClick={() => setSelectedGenre(section.genre)} className={`rounded-full border px-4 py-2 text-sm font-bold ${activeGenre === section.genre ? "border-primary bg-primary/15 text-primary" : "border-border hover:border-primary/50"}`}>{section.genre}</button>)}</div><h2 className="mt-8 text-2xl font-bold">{activeSection.genre}</h2><div data-testid="selected-genre-deals" className="mt-4 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-5">{activeSection.results.slice(0, 5).map((deal) => <DealCard key={`${deal.id ?? deal.steam_appid ?? deal.name}`} deal={deal} large />)}</div></section>}
  </AppShell>;
}
