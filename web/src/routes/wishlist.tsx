...
function WishlistCard({
  ...
  const priceQuery = useQuery({
    queryKey: ["wishlist", "price", item.catalog_game_id],
    queryFn: () => getGamePriceHistory(String(item.catalog_game_id)),
  });
  const price = priceQuery.data;
  const storeUrl = price?.current?.url ?? price?.url;
  return (
    <article className="grid grid-cols-1 gap-5 rounded-2xl border border-border bg-surface p-5 md:grid-cols-[auto_minmax(0,1fr)_auto] md:items-center">
      <GameCover
        from={item.cover_url ?? "#0f172a"}
        to="#0f172a"
        title={item.title}
        compact
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <Link
            to="/games/$gameId"
            params={{ gameId: String(item.catalog_game_id) }}
            className="truncate text-lg font-bold hover:text-primary"
          >
            {item.title}
          </Link>
          {price?.history_low_all &&
            price.current?.price &&
            price.current.price.amount <= price.history_low_all.amount && (
              <Chip tone="primary">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          {priceQuery.isLoading
            ...
            : priceQuery.isError
              ...
              : price?.current?.shop
                ? `Current offer at ${price.current.shop}`
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {alert ? (
            <Chip tone="primary">
              Alert:{" "}
              {alert.target_price
                ? `under ${alert.target_price}`
                : `-${alert.target_discount}%`}
            </Chip>
          ) : (
            <button
              onClick={onAlert}
      ...
      <div className="flex flex-wrap items-center gap-3 md:justify-end">
        <div className="text-right">
          <p className="font-mono text-2xl font-black text-primary">
            {money(price?.current?.price)}
          </p>
          {price?.current?.regular && (
            <p className="font-mono text-xs text-muted-foreground line-through">
              {money(price.current.regular)}
            </p>
          )}
        </div>
        {storeUrl && (
...
export function WishlistPage() {
  ...
  const [alertItem, setAlertItem] = useState<CatalogCollectionItem | null>(
    null,
  );
  ...
  const catalogQuery = useQuery({
    queryKey: ["wishlist", "catalog-search", search],
    queryFn: () => searchGames(search),
  ...
  const addWish = useMutation({
    mutationFn: ({
      ...
      title: string;
      cover?: string | null;
    }) => addWishlistItem(id, title, cover),
    onSuccess: refresh,
  });
  ...
  const removeWish = useMutation({
    mutationFn: (id: number) => removeWishlistItem(id),
    onSuccess: refresh,
  });
  ...
  const addAlert = useMutation({
    mutationFn: ({
      ...
      price?: number;
      discount?: number;
    }) => createPriceAlert(id, price, discount),
    onSuccess: async () => {
      setAlertItem(null);
      setTargetPrice("");
      setTargetDiscount("");
  ...
  const submitAlert = (event: FormEvent) => {
    ...
    const price = Number(targetPrice) || undefined;
    const discount = Number(targetDiscount) || undefined;
    if (alertItem && (price || discount))
      addAlert.mutate({ id: alertItem.catalog_game_id, price, discount });
  };
  const alertByGame = new Map(
    (alertsQuery.data ?? []).map((alert) => [
      alert.wishlist_catalog_game_id,
      alert,
    ]),
  );
  ...
  if (!signedIn)
    return (
      <AppShell>
        <SectionHeader
          title="Wishlist"
          hint="Sign in to save catalog games and price alerts."
        />
  ...
  return (
    <AppShell>
      <SectionHeader
        ...
        hint={
          wishlistQuery.isLoading
            ...
            : `${wishlistQuery.data?.length ?? 0} saved games`
        }
      />
      <section className="mb-8">
        <div className="flex items-center gap-3 rounded-xl border border-border bg-surface px-4 py-3">
          ...
          <input
            ...
            placeholder="Find a catalog game to save"
      ...
      {alertItem && (
        <section className="mb-6 rounded-2xl border border-primary/30 bg-surface p-5">
          <SectionHeader
            title={`Alert for ${alertItem.title}`}
            hint="Set a target price or a minimum discount. In-app delivery is always enabled."
          />
          <form onSubmit={submitAlert} className="flex flex-wrap gap-3">
            <input
              value={targetPrice}
              onChange={(event) => setTargetPrice(event.target.value)}
              inputMode="decimal"
              placeholder="Target price"
              className="w-40 rounded-lg border border-border bg-secondary px-3 py-2 text-sm outline-none"
            />
            <input
              ...
              className="w-32 rounded-lg border border-border bg-secondary px-3 py-2 text-sm outline-none"
            />
            <button
              disabled={(!targetPrice && !targetDiscount) || addAlert.isPending}
      ...
      <section className="space-y-4">
        ...
        {wishlistQuery.isSuccess && wishlistQuery.data.length === 0 && (
          <p className="rounded-2xl border border-border bg-surface p-5 text-sm text-muted-foreground">
            Search the catalog above to save your first game.
          </p>
        )}
        {wishlistQuery.data?.map((item) => (
          <WishlistCard
            key={item.catalog_game_id}
            item={item}
            alert={alertByGame.get(item.catalog_game_id)}
            onRemove={() => removeWish.mutate(item.catalog_game_id)}
            onAlert={() => setAlertItem(item)}
          />
        ))}
      </section>
      <section className="mt-10">
        <SectionHeader
          title="Favorites"
          hint={`${favoritesQuery.data?.length ?? 0} favorites`}
        />
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {favoritesQuery.data?.map((item) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 p-3">
                ...
                <button
                  onClick={() => removeFav.mutate(item.catalog_game_id)}
                  aria-label={`Remove favorite ${item.title}`}
          ...
          {favoritesQuery.isSuccess && favoritesQuery.data.length === 0 && (
            <p className="col-span-full rounded-xl border border-border bg-surface p-4 text-sm text-muted-foreground">
              Use the heart next to a catalog search result to save favorites.
            </p>
          )}
        </div>
      </section>
