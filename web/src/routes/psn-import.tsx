...
export function mergePreviewRows(items: PsnImportPreviewItem[], previous: Row[] = []): Row[] {
  ...
  return items.map((item) => {
    ...
    return {
      ...item,
      decision: keepManualCatalog
        ? "catalog"
        : recommendedDecision(item) === "skip"
          ? "raw"
          : recommendedDecision(item),
  ...
  return (
    <ol className="mb-8 flex flex-wrap gap-2">
      {steps.map((item, index) => (
        <li
          key={item.key}
          className={`label-mono rounded-full border px-3 py-1.5 ${index === active ? "border-primary/40 bg-primary/15 text-primary" : index < active ? "border-border bg-foreground/5" : "border-border text-muted-foreground"}`}
        >
          {index < active ? <Check className="mr-1 inline size-3" /> : `${index + 1}. `}
          {item.label}
        </li>
      ))}
    </ol>
...
function PsnImportPage() {
  ...
  const preview = useMutation({
    ...
    onSuccess: (data) => {
      setRows((current) => mergePreviewRows(data.items, current));
      setStep("preview");
      setError(null);
    },
    onError: (value) => setError(value instanceof Error ? value.message : "Could not read export."),
  });
  ...
  const decisions: PsnImportSelection[] = rows.flatMap((row) =>
    row.decision === "catalog" && row.catalogId
      ...
      : row.decision === "raw"
        ? [{ candidate_token: row.candidate_token, action: "raw" as const }]
        : [],
  );
  ...
  const selectAllGames = () =>
    setRows((current) =>
      current.map((row) =>
        row.status === "suggested_skip" && !row.restored
          ? row
          : {
              ...row,
              decision: row.recommended_action === "catalog" && row.igdb_id ? "catalog" : "raw",
              catalogId: row.igdb_id ?? null,
            },
      ),
    );
  const clearSelection = () =>
    setRows((current) => current.map((row) => ({ ...row, decision: "skip" })));
  const retryCatalog = () => uploadedFile && preview.mutate(uploadedFile);

  function previewRow(row: Row) {
    const badge =
      row.status === "catalog_unavailable"
        ? "Catalog temporarily unavailable"
        : row.decision === "catalog"
          ? "Catalog match"
          : "PSN title";
    return (
      <div key={row.candidate_token} className="rounded-xl border border-border bg-surface-2 p-4">
        <div className="flex items-start gap-3">
          <input
            ...
            onChange={() =>
              update(row.candidate_token, {
                decision: row.decision === "skip" ? (row.catalogId ? "catalog" : "raw") : "skip",
              })
            }
            className="mt-1 size-4 accent-[var(--primary)]"
          />
          <div className="min-w-0 flex-1">
            <p className="font-bold">{row.title ?? row.source_title}</p>
            <p className="label-mono mt-1 text-muted-foreground">{row.source_title}</p>
            {row.reason && <p className="mt-1 text-xs text-muted-foreground">{row.reason}</p>}
          </div>
          <Chip tone={row.decision === "skip" ? "outline" : "primary"}>{badge}</Chip>
        </div>
          ...
          <div className="mt-3 flex flex-wrap gap-2 border-t border-border pt-3">
            {row.suggestions.map((item) => (
              <button
                key={item.id}
                onClick={() =>
                  update(row.candidate_token, { decision: "catalog", catalogId: item.id })
                }
            ...
            <button
              onClick={() => update(row.candidate_token, { decision: "raw" })}
              className="rounded-lg border border-border px-2 py-1 text-xs"
            >
              Import as PSN title
            </button>
    ...
    <AppShell>
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <p className="mt-2 max-w-xl text-sm text-muted-foreground">
          Upload your PlayStation export, review its matches, and decide exactly what to add.
        </p>
        <div className="mt-8">
          <Stepper step={step} />
        </div>
        ...
        {step === "preview" && (
          <Panel className="p-6">
            ...
            />
            <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-surface-2 p-3">
              <p className="text-sm text-muted-foreground">
                Plausible games are selected automatically; catalog results only add details.
              </p>
              <div className="flex gap-2">
                ...
                <button
                  disabled={!uploadedFile || preview.isPending}
                  onClick={retryCatalog}
                  className="rounded-lg border border-border px-3 py-1.5 text-sm font-bold"
                >
                  Retry catalog enrichment
                </button>
              </div>
            </div>
            <div className="space-y-6">
              <section aria-label="Games to import">
                <h2 className="mb-2 text-sm font-bold">Games to import ({gameRows.length})</h2>
                {gameRows.map(previewRow)}
              </section>
              <section aria-label="Suggested non-games">
                <details>
                  <summary className="cursor-pointer text-sm font-bold">
                    Suggested non-games ({suggestedNonGames.length})
                  </summary>
                  <div className="mt-2 space-y-2">
                    {suggestedNonGames.map((row) => (
                      <div
                        key={row.candidate_token}
                        className="rounded-xl border border-border p-4"
                      >
                        <p className="font-bold">{row.source_title}</p>
                        <p className="text-xs text-muted-foreground">{row.reason}</p>
                        <button
                          onClick={() =>
                            update(row.candidate_token, { restored: true, decision: "raw" })
                          }
            ...
            <div className="mt-6 flex items-center justify-between">
              <p className="label-mono text-muted-foreground">
                {selectedCount} selected · {rows.length - selectedCount} skipped
              </p>
              <div className="flex gap-2">
                <button
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {decisions.filter((item) => item.action === "catalog").length}
                  </p>
                </div>
                <div>
                  <p className="label-mono text-muted-foreground">PSN titles</p>
                  <p className="text-2xl font-bold">
                    {decisions.filter((item) => item.action === "raw").length}
                  </p>
                </div>
                <div>
        ...
        {step === "result" && (
          <Panel className="p-6 text-center">
            ...
            <h2 className="mt-3 text-2xl font-bold">Import complete</h2>
            ...
            <div className="mt-6 flex justify-center gap-2">
              ...
              <button
                onClick={returnToUpload}
                className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-2 font-bold"
              >
                <RotateCcw className="size-4" /> Import another
              </button>
            </div>
          </Panel>
        )}
