...
function PsnImportPage() {
  ...
  const preview = useMutation({ mutationFn: previewPsnImport, onSuccess: data => { setRows(data.items.map(item => ({ ...item, decision: item.status === "matched" ? "catalog" : "skip", catalogId: item.igdb_id ?? null, restored: false }))); setStep("preview"); setError(null); }, onError: value => setError(value instanceof Error ? value.message : "Could not read export.") });
  ...
  const mapping = rows.filter(row => row.status === "needs_mapping" || row.restored);
  const skipped = rows.filter(row => row.status === "suggested_skip" && !row.restored);
  ...
  const returnToUpload = () => { setRows([]); setFileName(null); setError(null); if (input.current) input.current.value = ""; setStep("upload"); };

  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key !== "Escape") return;
      if (step === "confirm") setStep("preview");
      else if (step === "preview" || step === "result") returnToUpload();
    };
  ...
  function previewRow(row: Row, editable = false) {
    return <div key={row.candidate_token} className="rounded-xl border border-border bg-surface-2 p-4"><div className="flex items-start gap-3"><input aria-label={`Select ${row.source_title}`} type="checkbox" checked={row.decision !== "skip"} onChange={() => update(row.candidate_token, { decision: row.decision === "skip" ? (row.catalogId ? "catalog" : "raw") : "skip" })} className="mt-1 size-4 accent-[var(--primary)]" /><div className="min-w-0 flex-1"><p className="font-bold">{row.title ?? row.source_title}</p><p className="label-mono mt-1 text-muted-foreground">{row.source_title}</p>{row.reason && <p className="mt-1 text-xs text-muted-foreground">{row.reason}</p>}</div><Chip tone={row.decision === "skip" ? "outline" : "primary"}>{row.decision === "raw" ? "PSN title" : row.decision === "catalog" ? "Catalog match" : "Skipped"}</Chip></div>{editable && <div className="mt-3 flex flex-wrap gap-2 border-t border-border pt-3">{row.suggestions.map(item => <button key={item.id} onClick={() => update(row.candidate_token, { decision: "catalog", catalogId: item.id })} className="rounded-lg border border-border px-2 py-1 text-xs">Use {item.title}</button>)}<button onClick={() => update(row.candidate_token, { decision: "raw" })} className="rounded-lg border border-border px-2 py-1 text-xs">Import as PSN title</button><button onClick={() => update(row.candidate_token, { decision: "skip" })} className="rounded-lg border border-border px-2 py-1 text-xs">Skip</button></div>}</div>;
  }

  return <AppShell><Link to="/account" className="mb-5 inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground"><ArrowLeft className="size-4" /> Back to profile</Link><div className="mx-auto max-w-3xl"><p className="label-mono mb-3 text-primary">PlayStation</p><h1 className="text-4xl font-bold tracking-[-0.03em]">Import your library</h1><p className="mt-2 max-w-xl text-sm text-muted-foreground">Upload your PlayStation export, review its matches, and decide exactly what to add.</p><div className="mt-8"><Stepper step={step} /></div>{error && <InlineError>{error}</InlineError>}{step === "upload" && <Panel className="p-6"><SectionHeader title="Upload export" hint="XLSX, CSV or JSON, up to 10 MB" /><button onClick={() => input.current?.click()} className="flex w-full flex-col items-center gap-3 rounded-2xl border border-dashed border-border bg-surface-2 px-6 py-14"><FileUp className="size-6 text-primary" /><span className="font-bold">Choose an export file</span><span className="text-xs text-muted-foreground">.xlsx, .csv or .json</span></button><input ref={input} type="file" accept=".xlsx,.csv,.json" className="hidden" onChange={event => { const file = event.target.files?.[0]; if (file) { setFileName(file.name); preview.mutate(file); } }} />{preview.isPending && <p className="mt-4 flex items-center gap-2"><Loader2 className="size-4 animate-spin" /> Reading {fileName}…</p>}</Panel>}{step === "preview" && <Panel className="p-6"><SectionHeader title="Preview games" hint={fileName ? `Parsed from ${fileName}` : undefined} /><div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-surface-2 p-3"><p className="text-sm text-muted-foreground">Suggested skips remain excluded until you restore them.</p><div className="flex gap-2"><button onClick={selectAllGames} className="rounded-lg border border-border px-3 py-1.5 text-sm font-bold">Select all games</button><button onClick={clearSelection} className="rounded-lg border border-border px-3 py-1.5 text-sm font-bold">Clear selection</button></div></div><div className="space-y-6"><section aria-label="Catalog matches"><h2 className="mb-2 text-sm font-bold">Catalog matches ({matched.length})</h2>{matched.map(row => previewRow(row))}</section><section aria-label="Need mapping"><h2 className="mb-2 text-sm font-bold">Need mapping ({mapping.length})</h2>{mapping.map(row => previewRow(row, true))}</section><section aria-label="Suggested skip"><details><summary className="cursor-pointer text-sm font-bold">Suggested skip ({skipped.length})</summary><div className="mt-2 space-y-2">{skipped.map(row => <div key={row.candidate_token} className="rounded-xl border border-border p-4"><p className="font-bold">{row.source_title}</p><p className="text-xs text-muted-foreground">{row.reason}</p><button onClick={() => update(row.candidate_token, { restored: true, decision: "skip" })} className="mt-2 rounded-lg border border-border px-2 py-1 text-xs">Restore to mapping</button></div>)}</div></details></section></div><div className="mt-6 flex items-center justify-between"><p className="label-mono text-muted-foreground">{selectedCount} selected · {rows.length - selectedCount} skipped</p><div className="flex gap-2"><button onClick={returnToUpload} className="rounded-xl border border-border px-4 py-2 text-sm font-bold">Back</button><button disabled={!selectedCount} onClick={() => setStep("confirm")} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-bold text-primary-foreground disabled:opacity-50">Continue <ArrowRight className="size-4" /></button></div></div></Panel>}{step === "confirm" && <Panel className="p-6"><SectionHeader title="Confirm import" hint="Existing library entries won't be duplicated" /><div className="rounded-xl border border-border bg-surface-2 p-5"><div className="grid grid-cols-3 gap-4"><div><p className="label-mono text-muted-foreground">Catalog</p><p className="text-2xl font-bold">{decisions.filter(item => item.action === "catalog").length}</p></div><div><p className="label-mono text-muted-foreground">PSN titles</p><p className="text-2xl font-bold">{decisions.filter(item => item.action === "raw").length}</p></div><div><p className="label-mono text-muted-foreground">Skipped</p><p className="text-2xl font-bold">{rows.length - selectedCount}</p></div></div></div><div className="mt-6 flex justify-end gap-2"><button onClick={() => setStep("preview")} className="rounded-xl border border-border px-4 py-2 text-sm font-bold">Back</button><button onClick={() => { if (decisions.some(item => item.action === "raw") && !window.confirm("Import selected PSN titles without a catalog match?")) return; confirm.mutate(decisions); }} className="rounded-xl bg-primary px-4 py-2 text-sm font-bold text-primary-foreground">Import {selectedCount} games</button></div></Panel>}{step === "result" && <Panel className="p-6 text-center"><Check className="mx-auto size-8 text-primary" /><h2 className="mt-3 text-2xl font-bold">Import complete</h2><p className="mt-2 text-sm text-muted-foreground">Your selected PlayStation games are now in your library.</p><div className="mt-6 flex justify-center gap-2"><Link to="/library" className="rounded-xl bg-primary px-4 py-2 font-bold text-primary-foreground">Open library</Link><button onClick={returnToUpload} className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-2 font-bold"><RotateCcw className="size-4" /> Import another</button></div></Panel>}</div></AppShell>;
}
