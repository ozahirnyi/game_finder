...
export const lovableQueryKeys = {
  ...
  gamePriceHistory: (id: string, country: string) =>
    ["prices", "game", id, country] as const,
  upcomingGames: (pageSize: number) =>
    ["catalog", "upcoming", pageSize] as const,
  trendingGames: (pageSize: number) =>
    ["catalog", "trending", pageSize] as const,
  steamLibrary: ["steam", "library"] as const,
  steamSocial: (friendsLimit: number) =>
    ["steam", "social", friendsLimit] as const,
...
export function toSavedGameCard(game: SavedGame): LovableSavedGameCard {
  return {
    ...
    imageUrl:
      game.img_icon_url && game.external_id
