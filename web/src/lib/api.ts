...
export type PriceAlert = {
  ...
  identity_value: string;
  title: string;
  mode: "target_price" | "target_discount" | "any_discount";
...
export function listWishlist() {
  return request<WishlistItem[]>("/wishlist", { auth: true });
}
export function createWishlistItem(
  data: Pick<WishlistItem, "identity_kind" | "identity_value" | "title">,
) {
...
export function createPriceAlert(
  data: Omit<PriceAlert, "id" | "created_at" | "updated_at">,
) {
...
export function deletePriceAlert(id: string) {
  return request<void>(`/price-alerts/${id}`, { method: "DELETE", auth: true });
}
export function listNotifications() {
  return request<Notification[]>("/notifications", { auth: true });
}
