...
describe("GameFinder friends", () => {
  beforeEach(() => {
    ...
    api.getSteamSocial.mockResolvedValue({ steam: { linked: false }, friends: [], top_friend_games: [], public_libraries: 0, private_libraries: 0 });
    ...
    api.getSocialInviteLink.mockResolvedValue({ url: "https://gamefinder.example/friends?add=player" });
  });

  it("searches registered players and sends a confirmation-based friend request", async () => {
    api.searchUsers.mockResolvedValue([{ id: "user-2", display_name: "Niko", bio: "Co-op player", avatar: null }]);
    api.createFriendRequest.mockResolvedValue({ id: "request-1" });
    renderPage();

    fireEvent.change(await screen.findByPlaceholderText(/search gamefinder players/i), { target: { value: "Ni" } });
    expect(await screen.findByText("Niko")).toBeVisible();
    fireEvent.click(screen.getByRole("button", { name: /add niko/i }));

    await waitFor(() => expect(api.createFriendRequest).toHaveBeenCalledWith("user-2"));
  });
});
