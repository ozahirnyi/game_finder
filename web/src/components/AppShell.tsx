  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/30">
      <aside className="fixed left-0 top-0 z-40 hidden h-screen w-64 shrink-0 flex-col border-r border-border bg-surface p-6 lg:flex">
        <Link to="/" className="mb-10 flex items-center gap-3 px-2">
          <span className="grid size-8 place-items-center rounded-lg bg-primary">
            <span className="size-4 rounded-sm bg-background" />
          </span>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 flex items-center justify-around border-t border-border bg-surface/90 px-2 py-2 backdrop-blur lg:hidden">
        <Navigation nav={nav.slice(0, 5)} pathname={pathname} compact />
      </nav>
    </div>
  );
}
...
function Navigation({
  ...
  return (
    <nav className={compact ? "flex w-full justify-around" : "space-y-1"}>
      {nav.map((item) => {
        const active =
          ...
          <Link
            ...
            className={
              compact
                ...
                : `flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium ${active ? "bg-white/5 text-foreground" : "text-muted-foreground hover:text-foreground"}`
            }
          >
