...
describe("discovery API regions", () => {
  ...
  it("renders search results from api.ts and retries a failed request", async () => {
    vi.mocked(searchGames)
      ...
      .mockResolvedValueOnce({
        results: [
          { id: 1, name: "Hades II", released: null, background_image: null },
        ],
      });
  ...
  it("shows the fallback cover when a result has no background image", async () => {
    ...
    render(<DealsScreen />);
    expect(
      await screen.findByLabelText("Hades II cover unavailable"),
    ).toBeVisible();
  });
  ...
  it("renders trending and upcoming games from their API regions", async () => {
    ...
    vi.mocked(getUpcomingGames).mockResolvedValue({
      results: [
        {
          id: 2,
          name: "Upcoming game",
    ...
    expect(
      await screen.findByRole("heading", { name: "Trending game" }),
    ).toBeVisible();
    expect(
      await screen.findByRole("heading", { name: "Upcoming game" }),
    ).toBeVisible();
  });

  it("retries only the failed discovery region", async () => {
    vi.mocked(getTrendingGames)
      .mockRejectedValueOnce(new Error("offline"))
    ...
    expect(
      await screen.findByRole("heading", { name: "Stable upcoming" }),
    ).toBeVisible();
  ...
  it("keeps successful price history visible when the catalog request fails", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: "Retry game details" }),
    );
    expect(
      await screen.findByRole("heading", { name: "Hades II" }),
    ).toBeVisible();
  ...
  it("updates search results when the Vite query changes", async () => {
    ...
    vi.mocked(searchGames).mockResolvedValue({
      results: [
        { id: 2, name: "Celeste", released: null, background_image: null },
      ],
    });
    view.rerender(<SearchScreen key="celeste" initialQuery="celeste" />);
    expect(
