...
function GameRegion({ title, games, retryLabel, onRetry }: { title: string; games: RemoteState<SearchGame[]>; retryLabel: string; onRetry: () => void }) {
  ...
  return <div className="game-grid">{games.data.map((game) => <article key={game.id ?? game.name} className="game-card"><GameCover title={game.name ?? "Game"} src={game.background_image} /><h3>{game.name ?? "Untitled game"}</h3>{game.id ? <Link to="/games/$gameId" params={{ gameId: String(game.id) }}>View details</Link> : null}</article>)}</div>;
}
