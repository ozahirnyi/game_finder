...
const money = (value: HomeDeal["current"] extends infer T ? T : never) => value?.price ? `${value.price.amount} ${value.price.currency}` : "Price unavailable";
export function DealsScreen() {
  const [state, setState] = useState<RemoteState<HomeDeal[]>>({ status: "loading" }); const [retry, setRetry] = useState(0);
  useEffect(() => { let active = true; void getHomepageDeals("US", 12).then((data) => active && setState({ status: "success", data: data.results })).catch(() => active && setState({ status: "error", message: "Deals could not be loaded." })); return () => { active = false; }; }, [retry]);
