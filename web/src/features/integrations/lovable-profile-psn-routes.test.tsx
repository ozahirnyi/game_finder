...
describe("Lovable profile and PSN routes", () => {
  ...
  it("shows a retry when loading the current user fails", async () => {
    api.getCurrentUser.mockRejectedValueOnce(new Error("Profile unavailable")).mockResolvedValueOnce({ id: "user-1", email: "player@example.com", created_at: "2026-01-01", google_linked: false });
    render(<ProfileRoute.options.component />);
  ...
  it("retries the failed Google and Telegram link actions", async () => {
    api.getGoogleLinkUrl.mockRejectedValueOnce(new Error("Google link unavailable")).mockResolvedValueOnce({ url: "https://google.example/link" });
    api.getTelegramLinkUrl.mockRejectedValueOnce(new Error("Telegram link unavailable")).mockResolvedValueOnce({ configured: true, url: "https://t.me/example", message: null });
    const open = vi.spyOn(window, "open").mockImplementation(() => null);
    render(<ProfileRoute.options.component />);

    const googleRow = (await screen.findByText("Google")).parentElement?.parentElement!;
    const telegramRow = screen.getByText("Telegram").parentElement?.parentElement!;
    ...
    fireEvent.click(within(googleRow).getByRole("button", { name: "Retry Google" }));
    await waitFor(() => expect(api.getGoogleLinkUrl).toHaveBeenCalledTimes(2));
    fireEvent.click(within(telegramRow).getByRole("button", { name: "Retry Telegram" }));
    await waitFor(() => expect(api.getTelegramLinkUrl).toHaveBeenCalledTimes(2));
  ...
  it("retries a failed PSN preview with the already-selected file", async () => {
    const file = new File(["sheet"], "games.xlsx", { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
    api.previewPsnImport.mockRejectedValueOnce(new Error("Preview unavailable")).mockResolvedValueOnce({ games: ["Bloodborne"], total: 1, message: null });
    render(<PsnRoute.options.component />);

    fireEvent.change(screen.getByLabelText("Choose PSN export"), { target: { files: [file] } });
  ...
  it("renders unavailable placeholders for unsupported PSN account data", () => {
    ...
    expect(screen.getAllByText("Data unavailable").length).toBeGreaterThanOrEqual(10);
