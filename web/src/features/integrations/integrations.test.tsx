...
describe("integration screens", () => {
  beforeEach(() => {
    ...
    api.getSteamAccount.mockResolvedValue({
      linked: false,
      steam_id: null,
      persona_name: null,
      avatar: null,
    ...
    api.getSteamLoginUrl.mockResolvedValue({
      url: "https://steam.example/connect",
    });
    api.getSteamLibrary.mockResolvedValue({
      steam: {
        linked: true,
        steam_id: "1",
        persona_name: "Ada",
  ...
  it("uses the API-provided Telegram link URL", async () => {
    ...
    api.getTelegramLinkUrl.mockResolvedValue({
      configured: true,
  ...
  it("mounts the PSN screen at the PSN route instead of the available-soon placeholder", () => {
    ...
    render(<PsnPage />);
    expect(
      screen.getByText("Sign in to import PlayStation games"),
    ).toBeVisible();
    expect(
      screen.queryByText(
        "PlayStation Network imports will be available here soon.",
      ),
  ...
  it("renders returned Steam games and recommendations", async () => {
    ...
    api.getSteamRecommendations.mockResolvedValue({
      recommendations: [
        { title: "Portal", reason: "Puzzle favorite", tags: ["Puzzle"] },
      ],
    });
    render(<SteamScreen />);
  ...
  it("renders Steam social data returned by the API", async () => {
    ...
    expect(
      screen.getByText("3 public friend libraries available."),
    ).toBeVisible();
  });
  ...
  it("keeps the linked Steam library visible when social data fails and retries only social data", async () => {
    ...
    api.getSteamSocial
      .mockRejectedValueOnce(new Error("Friends service unavailable"))
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Retry friends' games" }),
    );
    expect(await screen.findByText("Deep Rock Galactic")).toBeVisible();
  ...
  it("keeps Steam library and social data visible when recommendations fail and retries recommendations", async () => {
    ...
    api.getSteamRecommendations
      .mockRejectedValueOnce(new Error("Recommendations unavailable"))
      .mockResolvedValueOnce({
        recommendations: [
          { title: "Portal", reason: "Puzzle favorite", tags: ["Puzzle"] },
        ],
    ...
    expect(
      await screen.findByText("Recommendations are unavailable"),
    ).toBeVisible();
    expect(screen.getByText("Half-Life")).toBeVisible();
    expect(screen.getByText("Deep Rock Galactic")).toBeVisible();
    fireEvent.click(
  ...
  it("keeps the core profile visible when optional integration regions fail and retries each region", async () => {
    ...
    expect(
      screen.queryByText("Telegram alerts are unavailable"),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "player@example.com" }),
    ).toBeVisible();
  });

  it("keeps profile and Google visible when connecting Telegram fails", async () => {
    api.getTelegramLinkUrl.mockRejectedValue(
      new Error("Telegram link unavailable"),
    );
    ...
    expect(
      await screen.findByText("Telegram alerts are unavailable"),
    ).toBeVisible();
    expect(
      screen.getByRole("heading", { name: "player@example.com" }),
    ).toBeVisible();
  ...
  it("keeps profile and Google visible when a Telegram test alert fails", async () => {
    ...
    api.sendTelegramTestAlert.mockRejectedValue(
  ...
  it("keeps profile and Google visible when unlinking Telegram fails", async () => {
    ...
    api.unlinkTelegramAccount.mockRejectedValue(
      new Error("Telegram unavailable"),
    );
