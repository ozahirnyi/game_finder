...
It is conceptually similar to **movie_finder** (Django + PostgreSQL + external APIs), but this stack is **FastAPI + PostgreSQL + Redis** and the domain is **video games**, with IGDB as catalog metadata.
...
1. **Standalone CLI script** — `httpx` + IGDB, key in `.env`, error handling.
...
5. **IGDB + Redis** — external search + cache.
