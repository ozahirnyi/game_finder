...
from app.psn_library_enrichment import (
    ...
    PsnInvalidCatalogGame,
    _link_psn_game_to_catalog,
    _merge_game_psn_catalog_evidence,
    _merge_psn_catalog_evidence,
    _pending_psn_catalog_query,
    _psn_game_catalog_evidence,
    _psn_linked_game_payload,
    _stable_psn_evidence_values,
    enrich_pending_psn_catalog_batch,
)
...
def fixture_peer(viewer: User | None, target: User) -> bool:
    key = getattr(viewer, "e2e_fixture_key", None)
    return bool(key and key == getattr(target, "e2e_fixture_key", None))
...
def _psn_has_platform(game: dict, platform_name: str) -> bool:
...
async def enrich_psn_library_catalog(
    ...
    try:
        return await enrich_pending_psn_catalog_batch(
            db, current_user.id,
        ...
        db.rollback()
        raise HTTPException(status_code=502, detail="Catalog is temporarily unavailable") from exc
...
async def apply_psn_library_repair(data: PsnLibraryRepairApplyRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):  # pragma: no cover - API boundary
    ...
    try:
        for game in games:
            ...
            except PsnInvalidCatalogGame as exc:
...
def update_user_profile(
    ...
    if display_name and display_name != current_user.display_name:
        if db.query(User.id).filter(User.display_name == display_name, User.id != current_user.id).first():
