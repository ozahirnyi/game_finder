...
from sqlalchemy import CheckConstraint, create_engine, Date, String, DateTime, ForeignKey, Float, Integer, Index, JSON, Text, text, UniqueConstraint, func
...
class Game(Base):
    ...
    psn_search_aliases: Mapped[Optional[list[str]]] = mapped_column(JSON, nullable=True)
    psn_source_platforms: Mapped[Optional[list[str]]] = mapped_column(JSON, nullable=True)
    catalog_lookup_version: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
