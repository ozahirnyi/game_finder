# example.com HTTPS Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use `subagent-driven-development` (recommended) or `executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Serve the Game Finder API at canonical HTTPS address `example.com`, redirect `www.example.com`, and provision a renewable Let’s Encrypt certificate.
...
- `example.com` is canonical; `www.example.com` redirects to it.
...
**Interfaces:**
- Consumes: hostnames `example.com`, `www.example.com`; upstream `http://127.0.0.1:8000`.
...
- Create: `/etc/letsencrypt/live/example.com/` certificate material, managed by Certbot
...
- Produces: valid TLS for both hosts and canonical API traffic at `https://example.com`.
...
```powershell
rtk proxy nslookup example.com 1.1.1.1
rtk proxy nslookup www.example.com 1.1.1.1
```
...
```bash
sudo bash -c 'systemctl stop nginx; trap "systemctl start nginx" EXIT; certbot certonly --standalone --non-interactive --agree-tos --register-unsafely-without-email -d example.com -d www.example.com'
```

Expected: Certbot validates both names and creates `/etc/letsencrypt/live/example.com/fullchain.pem` and `privkey.pem`; the final template remains unmodified by Certbot.
...
```powershell
curl --fail --location --silent --show-error http://www.example.com/health
