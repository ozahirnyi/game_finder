...
**Goal:** Let authenticated PlayFinder users save real RAWG catalog games to their library and wishlist, connect Google and Steam accounts, and release the result to `https://playfinder.cc`.
...
- Produces: a documented, no-secret release procedure using the exact `playfinder.cc` callback paths.
...
def test_production_google_callback_is_same_origin_api() -> None:
    ...
    assert "GOOGLE_REDIRECT_URI=https://playfinder.cc/api/auth/google/callback" in readme
    assert "BACKEND_PUBLIC_URL=https://playfinder.cc/api" in readme
```
...
- Verify: `https://playfinder.cc/api/health`, `https://playfinder.cc/api/auth/google/status`
...
Back up the server file with owner-only permissions, then edit `/home/ec2-user/.game-finder.env` without printing it. Preserve all existing secret lines and set only the five variables from Task 8. Confirm the Google Cloud Console authorized redirect URI is exactly `https://playfinder.cc/api/auth/google/callback`; if console access is unavailable, stop and request that single user action before claiming OAuth is live.
...
Run: `rtk proxy curl -fsS https://playfinder.cc/api/health`

Run: `rtk proxy curl -fsS https://playfinder.cc/api/auth/google/status`

Expected: `{"status":"ok"}` and a JSON response where `configured` is `true`.

- [ ] **Step 7: Manually test the real production browser flows**

Verify on `https://playfinder.cc`: guest catalog details; authenticated library/wishlist idempotency; Google sign-in; Steam sign-in; Google linking from profile; Steam linking from `/steam`; and callback errors. Confirm no redirect includes `access_token` and no localhost callback is used.
