...
**Goal:** Serve the production frontend at `https://playfinder.cc` and the existing FastAPI API at the same origin under `/api/`, with no active Railway dependency.
...
- `https://playfinder.cc` is the sole public application origin; `www` redirects to it.
...
- Produces: README instructions for a Lightsail-only `playfinder.cc` deployment and a source tree with no active Railway URL.

- [ ] **Step 1: Add a documentation assertion test**

Create `tests/test_deployment_docs.py` that reads `README.md` and asserts it contains `https://playfinder.cc/api` and does not contain `railway.app` or `Railway`.
...
- Produces: public home page and API health under `playfinder.cc`.
...
```bash
curl --fail --silent --show-error https://playfinder.cc/
curl --fail --silent --show-error https://playfinder.cc/api/health
curl --fail --location --silent --show-error https://www.playfinder.cc/api/health
```
...
In Google Cloud Console, replace the authorized redirect URI with `https://playfinder.cc/api/auth/google/callback`. Verify the API produces that URI before asking the user to complete a real provider sign-in.
