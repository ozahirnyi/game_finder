...
- `https://example.com` was inspected with `Invoke-WebRequest`: it was serving the earlier shortened home implementation, not the full UI of the supplied archive.
...
6. Verify GitHub Actions Lightsail run and use `Invoke-WebRequest https://example.com` to confirm the generated asset hash and the full hero markup changed.
